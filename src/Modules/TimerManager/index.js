...
Manager.Update = async (ID, { Name, Duration, Type, ShowOnWeb }) => {
    ...
    if (typeof Duration === 'number' || Duration === null) { fields.push('Duration = ?'); params.push(Duration); }
    if (typeof Type === 'string') { fields.push('Type = ?'); params.push(Type); }
    if (typeof ShowOnWeb === 'boolean') { fields.push('ShowOnWeb = ?'); params.push(ShowOnWeb ? 1 : 0); }
...
Manager.Create = async (Type, Name, Description, Duration, TextAlert, AudioAlert, ShowOnWeb = true) => {
