            ...
            try {
                ...
                if (elapsed >= EMIT_MIN_INTERVAL_MS && !emitTimer) {
                    ...
                    emitTimer = setTimeout(() => {
                        emitTimer = null;
                        try { io.emit("timers:update", pendingPayload || []); } catch {}
                        lastEmitAt = Date.now();
                        pendingPayload = null;
                    }, wait);
                }
    ...
    Broadcast.on("TimersUpdated", timersUpdatedHandler);
        ...
        await new Promise((resolve, reject) => {
            ...
            server.listen(port, host, () => {
                ...
                try { Broadcast.emit('Notify', `Web dashboard listening on http://${host}:${port}`, 'success', 2000); } catch {}
                resolve();
            });
        });
        ...
        if (code === 'EADDRINUSE') {
            try { Broadcast.emit('Notify', `Web dashboard port ${await Settings.GetValue('WEB_DASHBOARD_PORT')} is in use.`, 'error'); } catch {}
        } else if (code === 'EADDRNOTAVAIL') {
            try { Broadcast.emit('Notify', `Invalid bind address: ${await Settings.GetValue('WEB_DASHBOARD_BIND')}`, 'error'); } catch {}
        }
...
Manager.Stop = async () => {
    try {
        ...
        try { if (io) io.removeAllListeners(); } catch (_) { /* ignore */ }
        io = null;
        app = null;
        server = null;
        started = false;
