        ...
        Validate: async (val, setting) => {
            ...
            try {
                ...
                await new Promise((resolve, reject) => {
                    ...
                    let done = false;
                    sock.once('error', (err) => { if (!done) { done = true; try { sock.close(); } catch {} reject(err); } });
                    ...
                    setTimeout(() => { if (!done) { done = true; try { sock.close(() => resolve()); } catch { resolve(); } } }, 150);
                });
            } catch (e) {
            ...
            try {
                ...
                await new Promise((resolve, reject) => {
                    ...
                    sock.bind({ address: v, port: 0, exclusive: true }, () => {
                        if (!done) { done = true; try { sock.close(() => resolve()); } catch { resolve(); } }
                    });
                    setTimeout(() => { if (!done) { done = true; try { sock.close(() => resolve()); } catch { resolve(); } } }, 150);
                });
                return [true, null];
            } catch (e) {
                if (e && e.code === 'EADDRNOTAVAIL') return [false, `Bind address ${v} is not available on this machine.`];
        ...
        Validate: async (val, setting) => {
            ...
            try {
                ...
                await new Promise((resolve, reject) => {
                    ...
                    let done = false;
                    srv.once('error', (err) => { if (!done) { done = true; try { srv.close(); } catch {} reject(err); } });
                    ...
                    setTimeout(() => { if (!done) { done = true; try { srv.close(() => resolve()); } catch { resolve(); } } }, 150);
                });
            } catch (e) {
                return [false, `Port ${n} appears to be in use.`];
            }
            ...
            try {
                ...
                await new Promise((resolve, reject) => {
                    ...
                    srv.listen({ host: v, port: 0, exclusive: true }, () => {
                        if (!done) { done = true; try { srv.close(() => resolve()); } catch { resolve(); } }
                    });
                    setTimeout(() => { if (!done) { done = true; try { srv.close(() => resolve()); } catch { resolve(); } } }, 150);
                });
                return [true, null];
