...
document.addEventListener("DOMContentLoaded", async () => {
	...
	if (btnShow && !btnShow.dataset.bound) {
		btnShow.addEventListener("click", async (e) => {
			...
			if (window.API && typeof window.API.SetMode === "function") {
				try { await window.API.SetMode("SHOW"); } catch {}
			} else {
				RenderMode("SHOW");
			}
	...
	if (btnEdit && !btnEdit.dataset.bound) {
		btnEdit.addEventListener("click", async (e) => {
			if (window.API && typeof window.API.SetMode === "function") {
				try { await window.API.SetMode("EDIT"); } catch {}
			} else {
				RenderMode("EDIT");
			}
			// Remove focus ring immediately
	...
	if (window.API && typeof window.API.GetMode === "function") {
		try { RenderMode(await window.API.GetMode()); } catch { RenderMode("SHOW"); }
	} else {
		RenderMode("SHOW");
	}
...
window.API.SetTimers(async (NewTimers) => {
	...
	const j = JSON.stringify(safe.map(t => ({
		ID: t.ID, Type: t.Type, Name: t.Name, Duration: t.Duration, Status: t.Status,
	...
	Timers = safe;
		...
		if (Timers.length === 0) {
			...
			if (!document.getElementById('EMPTY_STATE')) {
				...
				$("#EMPTY_CREATE_BTN").off("click").on("click", async () => { await CreateTimerAndEdit(); });
				$("#EMPTY_SETTINGS_BTN").off("click").on("click", async () => {
		...
		if (document.getElementById('EMPTY_STATE')) { $("#EMPTY_STATE").remove(); }
...
async function ProcessTimer(Timer) {
	...
	if (TimerElement.length === 0) {
		$("#APPLICATION_CONTENT").append(`<div class="SHOWTRAK_TIMER card p-2 ${Timer.Status}" id="TIMER_${Timer.ID}" data-timerid="${Timer.ID}" data-type="${Timer.Type}">
	...
	$ctr.off("click").on("click", "button.overlay-btn", async function(e){
			...
			try {
				...
				const status = ($card.hasClass('PAUSED') && 'PAUSED') || ($card.hasClass('RUNNING') && 'RUNNING') || ($card.hasClass('STANDBY') && 'STANDBY') || ($card.hasClass('COMPLETE') && 'COMPLETE');
				if (action === "start") {
					if (status === "PAUSED") await Unpause(id); else await Start(id);
	...
	const showBtn = (act, show) => $buttons.filter(`[data-action='${act}']`).toggleClass("d-none", !show);
	...
	const showEdit = AppMode === "EDIT";
	$(`#TIMER_CTRL_CONTAINER_${Timer.ID} button.overlay-btn[data-action='edit']`).toggleClass('d-none', !showEdit);
			...
			if (Setting.Type === "BOOLEAN") {
				...
				const applyOrEnable = async () => {
					if (!requiresSave) {
						...
						} catch (e) { Notify(String(e && e.message ? e.message : e), 'error'); }
					} else {
						$(`#${saveId}`).prop('disabled', !dirty);
					}
				};
				$(`#${id}`).off("input change").on("input change", function () {
				...
				if (requiresSave) {
					$(`#${saveId}`).off('click').on('click', async () => {
						try {
							const [err] = await window.API.SetSetting(Setting.Key, pendingVal);
							if (err) { Notify(err, 'error'); return; }
...
window.API.SetOSCList(async (Routes) => {
	...
	try {
		...
		const port = oscPortSetting && Number(oscPortSetting.Value) ? Number(oscPortSetting.Value) : 3333;
		const bind = oscBindSetting && String(oscBindSetting.Value || '').trim() ? String(oscBindSetting.Value).trim() : '0.0.0.0';
		$('#OSC_ROUTE_LIST_PORT_TEXT').text(`The following OSC routes are accessible on ${bind}:${port}.`);
	} catch { /* ignore */ }
...
async function Notify(Message, Type = "info", Duration = 5000) {
	...
	if (Duration && Duration > 0) {
		...
		const clear = () => { if (timerId) { clearTimeout(timerId); timerId = null; } };
		const tick = () => {
			clear();
			lastStart = Date.now();
			timerId = setTimeout(() => { try { el.remove(); } catch {} }, remaining);
		};
		const onMouseEnter = () => {
		...
		const onMouseLeave = () => {
			if (remaining === 0) { try { el.remove(); } catch {} }
			else { tick(); }
		};
...
async function OpenTimerEditModal(TimerID) {
	try {
		...
		const durationText = timer.Type === 'STOPWATCH' || timer.Duration == null ? "" : formatMsToHMS(timer.Duration);
		...
		toggleDuration();
		$("#TIMER_EDIT_SAVE").off('click').on('click', async () => {
		...
		$("#TIMER_EDIT_DELETE").off('click').on('click', async () => {
